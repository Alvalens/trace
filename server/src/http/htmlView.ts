...
function item(i: HandoverItem): string {
  const tag = i.flagType ? ` [${esc(i.flagType)}]` : '';
  const reason = i.reason ? `<div class="reason">${esc(i.reason)}</div>` : '';
  return `<li><b>${esc(i.title)}</b> <span class="meta">(${esc(i.status)}/${esc(i.classification)}${tag})</span>${reason}<div class="src">src: ${i.sourceIds.map((id) => esc(id)).join(', ')}</div></li>`;
}
...
<h1>Night-Shift Handover</h1><p class="meta">${esc(h.hotel)} · morning ${esc(h.date)} · ${esc(String(h.meta.eventsConsidered))} events${h.meta.proseNightIngested ? '' : ' · prose night not yet ingested'}</p>
